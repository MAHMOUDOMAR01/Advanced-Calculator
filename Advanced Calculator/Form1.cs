...
namespace Advanced_Calculator
{
    public partial class Form1 : Form
    {
        ...
        {
                txtDis.Text = $"Sin({txtBox.Text})";
                result = txtBox.Text = Convert.ToString(Math.Sin(Convert.ToDouble(txtBox.Text) * Math.PI / 180)); // تحويل الدرجة إلى راديان
            }

        private void button2_Click(object sender, EventArgs e)
        {
            txtDis.Text = $"Cos({txtBox.Text})";
            result = txtBox.Text = Convert.ToString(Math.Cos(Convert.ToDouble(txtBox.Text) * Math.PI / 180)); // تحويل الدرجة إلى راديان
        }

        private void button4_Click(object sender, EventArgs e)
        {
            txtDis.Text = $"Tan({txtBox.Text})";
            result = txtBox.Text = Convert.ToString(Math.Tan(Convert.ToDouble(txtBox.Text) * Math.PI / 180)); // تحويل الدرجة إلى راديان
        }

        private void button3_Click(object sender, EventArgs e)
        {
            txtDis.Text = $"Exp({txtBox.Text})";
            result = txtBox.Text = Convert.ToString(Math.Exp(Convert.ToDouble(txtBox.Text))); // حساب الأساس e
        }
